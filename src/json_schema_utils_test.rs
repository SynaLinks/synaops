...
#[test]
fn test_decompose_schema_plural_array() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    let result = decompose_schema(&schema);
    let props = result.get("properties").unwrap().as_object().unwrap();
    assert!(props.contains_key("query"));
    assert!(!props.contains_key("queries"));
    let query_prop = props.get("query").unwrap();
    assert_eq!(
        query_prop.get("type").unwrap().as_str(),
        Some("string"),
    );
}

#[test]
fn test_decompose_schema_non_plural_unchanged() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "answer": {
...
#[test]
fn test_decompose_schema_mixed() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
...
#[test]
fn test_decompose_schema_preserves_title() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    let result = decompose_schema(&schema);
    assert_eq!(result.get("title").unwrap().as_str(), Some("MyModel"));
}

#[test]
fn test_decompose_schema_singular_title() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    let query_prop = result
        ...
        .get("query")
        .unwrap();
    assert_eq!(query_prop.get("title").unwrap().as_str(), Some("Query"));
}

#[test]
fn test_decompose_schema_with_defs() {
    let schema = v(json!({
        "$defs": {
...
#[test]
fn test_decompose_schema_empty_defs_removed() {
    let schema = v(json!({
        "$defs": {},
        "additionalProperties": false,
        "properties": {
...
#[test]
fn test_decompose_schema_plural_non_array_unchanged() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    let result = decompose_schema(&schema);
    let props = result.get("properties").unwrap().as_object().unwrap();
    // Not an array type, so kept as-is
    assert!(props.contains_key("queries"));
}

#[test]
fn test_decompose_schema_required_updated() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    assert!(!required.iter().any(|v| v.as_str() == Some("queries")));
}

#[test]
fn test_decompose_schema_no_items_defaults_to_string() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "queries": {
    ...
    let query_prop = result
        ...
        .get("query")
        .unwrap();
    assert_eq!(query_prop.get("type").unwrap().as_str(), Some("string"));
}

#[test]
fn test_decompose_schema_items_with_ref() {
    let schema = v(json!({
        "$defs": {
            "Action": {"type": "object", "properties": {"name": {"type": "string"}}}
        },
        "additionalProperties": false,
        "properties": {
            "actions": {
    ...
    // Should have copied the $ref from items
    assert_eq!(
        action_prop.get("$ref").unwrap().as_str(),
        Some("#/$defs/Action"),
    );
}

#[test]
fn test_decompose_schema_roundtrip_with_factorize() {
    let schema = v(json!({
        "additionalProperties": false,
        "properties": {
            "query": {"title": "Query", "type": "string"},
            "query_0": {"title": "Query 0", "type": "string"}
        },
        "required": ["query", "query_0"],
        "title": "Test",
        "type": "object"
    }));
    let factorized = factorize_schema(&schema);
    let decomposed = decompose_schema(&factorized);
    let props = decomposed.get("properties").unwrap().as_object().unwrap();
    // Should have a singular "query" property
    assert!(props.contains_key("query"));
    assert_eq!(
        props.get("query").unwrap().get("type").unwrap().as_str(),
        Some("string"),
    );
}
